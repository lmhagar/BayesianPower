## code to approximate power curves for hypothesis tests with the predictive approach

## these functions were used to produce Figure 4
...
powerCurveGamma1 <- function(conviction, power, delta_L, delta_U, gamma_alpha.1, gamma_beta.1, gamma_alpha.2, 
  ...
  for (i in 1:nrow(sob_pwr)){
    power1 <- targetPower(targetP = 0, n_val = ceiling(mu_start),
                          params = c(gamma_alpha.1[i], gamma_beta.1[i], gamma_alpha.2[i], gamma_beta.2[i], threshold),
    ...
    while(length(last_group) > 0){
      ...
      for (i in 1:length(last_group)){
        endpoints1_vec[i] <- targetPower(targetP = 0, n_val = ceiling(upper_c*mu_high),
                                         params = c(gamma_alpha.1[last_group[i]], gamma_beta.1[last_group[i]], 
                                                    gamma_alpha.2[last_group[i]], gamma_beta.2[last_group[i]], threshold),
  ...
  for (i in 1:nrow(sob_pwr)){
    if (endpoints_vec[i] == 1){
      samps_pwr[i] <- uu(targetPower, lower = ceiling(mu_high), upper = ceiling(upper_c*mu_high), targetP = conviction, 
                         params = c(gamma_alpha.1[i], gamma_beta.1[i], 
                                    gamma_alpha.2[i], gamma_beta.2[i], threshold),
                         deltas = c(delta_L, delta_U), u = sob_pwr[i,], q = q)
    }
  }
    ...
    for (i in 1:length(inconsistent)){
      ...
      if (power_star[inconsistent[i]] < conviction){
        samps_pwr[inconsistent[i]] <- uu(targetPower, lower = n_star, upper = ceiling(upper_c*mu_start), 
                                         ...
                                         params = c(gamma_alpha.1[inconsistent[i]], gamma_beta.1[inconsistent[i]], 
                                                    gamma_alpha.2[inconsistent[i]], gamma_beta.2[inconsistent[i]], threshold),
  ...
  for (i in 1:nrow(sob_pwr)){
    ...
    if (power1 >= conviction){
      ...
      if (power1b >= conviction){
        ...
        samps_pwr[i] <- uu(targetPower, lower =ceiling(mu_low), upper = ceiling(mu_start), 
                           f_lower = power1b - conviction, f_upper = power1 - conviction, targetP = conviction,
                           params = c(gamma_alpha.1[i], gamma_beta.1[i], gamma_alpha.2[i], gamma_beta.2[i], threshold), hyper = hyper_mat,
                           deltas = c(delta_L, delta_U), u = sob_pwr[i,], q = q)
      }
    }
    else{
...
## generate draws from nondegenerate design prior using rejection sampling
informs <- read.csv("informs_gamma.csv")
set.seed(10)
a1s <- rgamma(1000000, informs[1,1], informs[1,2])
b1s <- rgamma(1000000, informs[2,1], informs[2,2])
a2s <- rgamma(1000000, informs[3,1], informs[3,2])
b2s <- rgamma(1000000, informs[4,1], informs[4,2])

t1s <- 1 - pgamma(4.29, a1s, b1s)
t2s <- 1 - pgamma(4.29, a2s, b2s)

ts <- t1s/t2s
inds_keep <- which(ts > 1/1.1 & ts < 1.1)
ts_sub <- ts[inds_keep]

params <- cbind(a1s, b1s, a2s, b2s, t1s, t2s, ts)

write.csv(params[inds_keep,], "sample_pD.csv", row.names = FALSE)
pD <- params[inds_keep,]
size_pD <- nrow(pD)

## Algorithm 1: Uninformative Priors for Figure 4
tic <- Sys.time()
for (k in 1:1){
  res_temp <- foreach(j=1:100, .combine='rbind', .packages = c("numDeriv", "qrng", "nleqslv"),
                   .options.snow=opts) %dopar% {
                     set.seed(2*j - 1)
                     inds_pD <- sample(size_pD, 4096, replace = FALSE)
                     pDa1 <- pD[inds_pD, 1]
                     pDb1 <- pD[inds_pD, 2]
                     pDa2 <- pD[inds_pD, 3]
                     pDb2 <- pD[inds_pD, 4]
                     unlist(powerCurveGamma1(conviction = convictions[k], power = powers[k], delta_L = delta_Ls[k],
                               delta_U = delta_Us[k], gamma_alpha.1 = pDa1, gamma_beta.1 = pDb1,
                               gamma_alpha.2 = pDa2, gamma_beta.2 = pDb2, threshold = 4.29, m = 4096, seed = 2*j))
                   }
  
  write.csv(res_temp[,1:4096], paste0("alg1_bpc_pred_samps_pwr_1", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4097:4099], paste0("alg1_bpc_pred_consistency_1", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4100:4249], paste0("alg1_bpc_pred_adjust_1", k, ".csv"), row.names = FALSE)
}
toc <- Sys.time()
toc - tic

## Algorithm 1: Informative Priors for Figure 4
tic <- Sys.time()
for (k in 1:1){
  res_temp <- foreach(j=1:100, .combine='rbind', .packages = c("numDeriv", "qrng", "nleqslv"),
                      .options.snow=opts) %dopar% {
                        ...
                        unlist(powerCurveGamma1(conviction = convictions[k], power = powers[k], delta_L = delta_Ls[k],
                                                delta_U = delta_Us[k], gamma_alpha.1 = pDa1, gamma_beta.1 = pDb1,
                                                gamma_alpha.2 = pDa2, gamma_beta.2 = pDb2, threshold = 4.29, m = 4096, seed = 200 + 2*j))
                      }
  
  write.csv(res_temp[,1:4096], paste0("alg1_bpc_pred_samps_pwr_2", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4097:4099], paste0("alg1_bpc_pred_consistency_2", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4100:4249], paste0("alg1_bpc_pred_adjust_2", k, ".csv"), row.names = FALSE)
}
toc <- Sys.time()
toc - tic

## Algorithm 2: Uninformative Priors for Figure 4
tic <- Sys.time()
for (k in 1:1){
  res_temp <- foreach(j=1:100, .combine='rbind', .packages = c("numDeriv", "qrng", "nleqslv"),
                      .options.snow=opts) %dopar% {
                        ...
                        unlist(powerCurveGamma2(conviction = convictions[k], power = powers[k], delta_L = delta_Ls[k],
                                                delta_U = delta_Us[k], gamma_alpha.1 = pDa1, gamma_beta.1 = pDb1,
                                                gamma_alpha.2 = pDa2, gamma_beta.2 = pDb2, threshold = 4.29, m = 4096, seed = 400 + 2*j,
                                                mu0.1 = 2, tau0.1 = 0.25, kappa0.1 = 2, lambda0.1 = 0.25, 
                                                mu0.2 = 2, tau0.2 = 0.25, kappa0.2 = 2, lambda0.2 = 0.25))
                      }
  
  write.csv(res_temp[,1:4096], paste0("alg2_bpc_pred_samps_pwr_1", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4097:4099], paste0("alg2_bpc_pred_consistency_1", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4100:4249], paste0("alg2_bpc_pred_adjust_1", k, ".csv"), row.names = FALSE)
}
toc <- Sys.time()
toc - tic

## Algorithm 2: Informative Priors for Figure 4
tic <- Sys.time()
for (k in 1:1){
  res_temp <- foreach(j=1:100, .combine='rbind', .packages = c("numDeriv", "qrng", "nleqslv"),
                      .options.snow=opts) %dopar% {
                        ...
                        unlist(powerCurveGamma2(conviction = convictions[k], power = powers[k], delta_L = delta_Ls[k],
                                                delta_U = delta_Us[k], gamma_alpha.1 = pDa1, gamma_beta.1 = pDb1,
                                                gamma_alpha.2 = pDa2, gamma_beta.2 = pDb2, threshold = 4.29, m = 4096, seed = 600 + 2*j,
  ...
  write.csv(res_temp[,1:4096], paste0("alg2_bpc_pred_samps_pwr_2", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4097:4099], paste0("alg2_bpc_pred_consistency_2", k, ".csv"), row.names = FALSE)
  write.csv(res_temp[,4100:4249], paste0("alg2_bpc_pred_adjust_2", k, ".csv"), row.names = FALSE)
}
toc <- Sys.time()
toc - tic

## The next section of code estimates the power curves for Figure 4 by simulating
...
convictions <- c(0.5)
powers <- c(0.6)
delta_Ls <- exp(c(log(1/(1+0.25))))
delta_Us <- exp(c(log(1 + 0.25)))
...
registerDoSNOW(cl)
pb <- txtProgressBar(max = 20000, style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)
...
for (k in 1:1){
  ...
  for (i in 3:length(rrr)){
    print(rrr[i])
    
    pwr_rep <- foreach(j=1:20000, .combine='rbind', .packages = c("rjags", "coda"),
                       .options.snow=opts) %dopar% {
                         
                         set.seed((i-1)*20000 + 800 + j)
                         ind_pD <- sample(size_pD, 1, replace = FALSE)
                         gamma_alpha.1 <- pD[ind_pD, 1]
                         gamma_beta.1 <- pD[ind_pD, 2]
                         gamma_alpha.2 <- pD[ind_pD, 3]
                         gamma_beta.2 <- pD[ind_pD, 4]
    ...
    write.csv(data.frame(rrr = rrr , results_rrr = results_rrr, results_rrr_ci), 
              paste0("pred_confirm_results_2", k, ".csv"), row.names = FALSE)
  }
}
...
for (k in 1:1){
  ...
  for (i in 1:length(rrr)){
    ...
    results_rrr_ci[i] <- mean(as.numeric(pwr_rep[,2]))
    write.csv(data.frame(rrr = rrr , results_rrr = results_rrr, results_rrr_ci), 
              paste0("pred_confirm_results_1", k, ".csv"), row.names = FALSE)
  }
}
...
for (k in c(1:1)){
  ...
  for (ii in 1:2){
    ...
    res_1 <- read.csv(paste0("alg1_bpc_pred_samps_pwr_", ii, k, ".csv"))
    res_2 <- read.csv(paste0("alg2_bpc_pred_samps_pwr_", ii, k, ".csv"))
    ## power curve approximations from simulating data from design distributions
    rrr <- read.csv(paste0("pred_confirm_results_",ii, k, ".csv"))$rrr
    results_rrr <- read.csv(paste0("pred_confirm_results_", ii, k, ".csv"))$results_rrr
    power <- powers[jj]
    y_mat <- NULL
    z_mat <- NULL
    for (i in 1:100){
...
legend = cowplot::get_plot_component(get(paste0("plot",ii, k, "legend")), 'guide-box-bottom', return_all = TRUE)
fig_final <- plot_grid(figp.row1, legend, ncol = 1, rel_heights = c(2/3, .1))
